...
macro_rules! fastboot_ack {
    ($ty:literal, $msg:expr) => {{
        ...
        transport_usbf_send(&buffer[..$msg.len() + 4]).map_err(|_| ())?;
...
// #[inline(always)]
pub fn fastboot_ack(ack_type: &[u8], message: &[u8]) -> Result<(), ()> {
    if message.len() > 60 {
        return Err(());
    }
    ...
    transport_usbf_send(&buffer[..message.len() + 4]).map_err(|_| ())?;

    Ok(())
}

pub fn fastboot_info(message: &[u8]) -> Result<(), ()> {
    fastboot_ack!(b"INFO", message)
}

pub fn fastboot_fail(message: &[u8]) -> Result<(), ()> {
    fastboot_ack!(b"FAIL", message)
}

pub fn fastboot_okay(message: &[u8]) -> Result<(), ()> {
    fastboot_ack!(b"OKAY", message)
}

pub fn fastboot_data(len: u32) -> Result<(), ()> {
