use crate::{
    fastboot::{
        externs::{transport_usbf_send, BlockDeviceType},
        fastboot_data, fastboot_fail, fastboot_info, fastboot_okay, FastbootCommandHandlerRes,
    },
    try_something,
};
...
pub fn fastboot_dump_brom(_arg: &[u8]) -> FastbootCommandHandlerRes {
    const BROM_ADDR: usize = 0x10000;
    const BROM_LEN: usize = 0x18000;
    crate::fastboot::externs::map_address_uncached(BROM_ADDR, BROM_LEN);

    if fastboot_data(BROM_LEN as u32).is_err() {
        return FastbootCommandHandlerRes::DropDevice;
    }

    let buffer = unsafe { core::slice::from_raw_parts(BROM_ADDR as *const u8, BROM_LEN) };
    if transport_usbf_send(&buffer).is_err() {
        return FastbootCommandHandlerRes::DropDevice;
    }
